...
@section('content')
    ...
    <form id="form-update" class="form-horizontal" method="POST"
          action="{{ route('conformity.update', [$course->id, $stage->id, $quiz->id, $question->id, $conformity->id]) }}"
        ...
        <div class="row flex-row">
            ...
            <div class="@if(!in_array($question->type, ['make_sentence', 'make_text', 'listen_write'])) col-8 @else col-12 @endif">
                @include('layouts.cms.template-parts.course.question.edit-matching-form')
            </div>
            ...
            <div class="col-4">
                ...
                @if($question->type === 'fill_the_gaps')
                    @include('cms.courses.quizzes.questions.conformity.layouts.edit.filling')
                @endif
                {{-- Single Choice --}}
                @if($question->type === 'single_choice')
                    @include('cms.courses.quizzes.questions.conformity.layouts.edit.single')
                @endif
                {{-- Multiple Choice--}}
                @if($question->type === 'multiple_choice')
                    @include('cms.courses.quizzes.questions.conformity.layouts.edit.multiple')
                @endif
                {{-- Logic Choice--}}
                @if($question->type === 'logic_choice')
                    @include('cms.courses.quizzes.questions.conformity.layouts.edit.logic')
                @endif
                {{-- Matching --}}
                @if($question->type === 'matching')
                    @include('cms.courses.quizzes.questions.conformity.layouts.edit.matching')
                @endif
                {{-- Short Answer --}}
                @if($question->type === 'short_answer')
                    @include('cms.courses.quizzes.questions.conformity.layouts.edit.short')
                @endif
            </div>
        </div>
